...
class AudioSpectrogramPlayer:
    ...
    def create_widgets(self):
        ...
        noise_frame = tk.LabelFrame(self.root, text="50Hz Netzbrummen-Simulation", 
                                    padx=10, pady=10)
        ...
        models = [
            ("Einfaches Sinus (Pure 50Hz)", "simple"),
        ...
        axis_frame = tk.LabelFrame(self.root, text="Spektrum-Achsen Einstellungen", 
                                   padx=10, pady=10)
        axis_frame.pack(pady=10, padx=20, fill=tk.X)
        
        # Auto-Scale Checkbox
        auto_frame = tk.Frame(axis_frame)
        auto_frame.pack(fill=tk.X, pady=5)
        
        self.auto_check = tk.Checkbutton(auto_frame, text="Automatische Skalierung", 
        ...
        # Apply Button
        apply_btn = tk.Button(axis_frame, text="Anwenden", command=self.apply_axis_settings,
                             width=15)
        ...
        # Initial: Manuelle Eingaben deaktivieren da Auto-Scale an ist
        self.toggle_auto_scale()
        
    def toggle_auto_scale(self):
        """Aktiviert/Deaktiviert manuelle Achsen-Eingaben"""
        if self.auto_scale.get():
            self.x_min_entry.config(state=tk.DISABLED)
            self.x_max_entry.config(state=tk.DISABLED)
            self.y_min_entry.config(state=tk.DISABLED)
            self.y_max_entry.config(state=tk.DISABLED)
        else:
            self.x_min_entry.config(state=tk.NORMAL)
            self.x_max_entry.config(state=tk.NORMAL)
            self.y_min_entry.config(state=tk.NORMAL)
            self.y_max_entry.config(state=tk.NORMAL)
        
        # Spektrum neu zeichnen mit neuen Einstellungen
    ...
    def generate_noise(self, duration, sample_rate):
        ...
        if self.noise_model.get() == "simple":
            ...
            # Modell 3: Trafo-Brummen mit Magnetostriktion
    ...
    def update_spectrum(self):
        ...
        if sample_pos + window_size < len(self.audio_data):
            ...
            # Achsen-Skalierung basierend auf Einstellungen
            if self.auto_scale.get():
                # Automatische Skalierung
                freq_max = min(8000, self.sample_rate / 2)
                self.ax_spectrum.set_xlim(0, freq_max)
                
                valid_magnitudes = magnitude_db[freqs <= freq_max]
                if len(valid_magnitudes) > 0:
                    y_min = np.percentile(valid_magnitudes, 5)
                    y_max = np.percentile(valid_magnitudes, 95)
                    margin = (y_max - y_min) * 0.1
                    self.ax_spectrum.set_ylim(y_min - margin, y_max + margin)
            else:
                # Manuelle Skalierung
                try:
            ...
            scale_mode = "Auto" if self.auto_scale.get() else "Manuell"
            self.ax_spectrum.set_title(f"Frequenzspektrum bei {time_str} ({scale_mode})", 
